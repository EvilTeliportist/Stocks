    ...
    with pyodbc.connect(
        ...
        with conn.cursor() as cursor:
            for ticker in tickers:
                s = "INSERT INTO " + ticker + " VALUES(" + t + ",%f)" % (price)
                # print(s)
                cursor.execute(s)
...
# Pulls raw data from the sql database and adds to a Pandas DF
def get_data(ticker, startDate, endDate):
    with pyodbc.connect(
            'DRIVER=' + driver + ';SERVER=' + server + ';PORT=1433;DATABASE=' + database + ';UID=' + username + ';PWD=' + password) as conn:
        with conn.cursor() as cursor:
            s = "EXEC [dbo.getStockData] @ticker ='"+ticker+"', @startDate='"+startDate+"', @endDate='"+endDate+"'"
            # print(s)
            cursor.execute(s)
            data = pd.DataFrame(cursor.fetchall(), columns=['Datetime', 'Price'])
            # print(data)
